...
class CDPBrowserManager:
    ...
    def _register_cleanup_handlers(self):
        ...
        def sync_cleanup():
            ...
            if self.launcher and self.launcher.browser_process:
                print("[CDPBrowserManager] atexit: 清理浏览器进程")
        ...
        def signal_handler(signum, frame):
            """信号处理器"""
            print(f"[CDPBrowserManager] 收到信号 {signum}，清理浏览器进程")
            if self.launcher and self.launcher.browser_process:
                self.launcher.cleanup()
        ...
        print("[CDPBrowserManager] 清理处理器已注册")
    ...
    async def _get_browser_path(self) -> str:
        ...
        if config.CUSTOM_BROWSER_PATH and os.path.isfile(config.CUSTOM_BROWSER_PATH):
            print(f"[CDPBrowserManager] 使用自定义浏览器路径: {config.CUSTOM_BROWSER_PATH}")
        ...
        print(f"自动检测浏览器路径: {browser_paths}")
        ...
        try:
            ...
            if self.browser_context:
                try:
                    ...
                    try:
                        ...
                        print("[CDPBrowserManager] 浏览器上下文已经被关闭")
                    ...
                    if "closed" not in error_msg and "disconnected" not in error_msg:
                        ...
                        print(f"[CDPBrowserManager] 浏览器上下文已关闭: {context_error}")
                finally:
            ...
            if self.browser:
                try:
                    ...
                    if self.browser.is_connected():
                        ...
                        print("[CDPBrowserManager] 浏览器连接已经断开")
                    ...
                    if "closed" not in error_msg and "disconnected" not in error_msg:
                        ...
                        print(f"[CDPBrowserManager] 浏览器连接已关闭: {browser_error}")
                finally:
                    self.browser = None
            ...
            if force or config.AUTO_CLOSE_BROWSER:
                if self.launcher and self.launcher.browser_process:
                    ...
                    print("[CDPBrowserManager] 没有需要清理的浏览器进程")
            else:
                print(
